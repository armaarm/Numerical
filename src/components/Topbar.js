...
const Topbar = () => {
    ...
    return ( 
        ...
        toggle = { RootOfEquationToggle } >
        <DropdownToggle color = "secondary" caret >
        Root of Equation { " " } </DropdownToggle>{" "} 
        <DropdownMenu>
        ...
        <DropdownItem href = "/OnePoint" > { " " }
        One - Point Iteration { " " } </DropdownItem>{" "} 
        <DropdownItem href = "/Secant" > Secant Method </DropdownItem>{" "} 
        <DropdownItem href = "/NewtonRaphson" > { " " }
        Newton Raphson { " " } </DropdownItem>{" "} 
        </DropdownMenu>{" "} 
        </Dropdown>{" "} 
        <span> &nbsp; &nbsp; </span>{" "} 
        <Dropdown isOpen = { LinearMenu }
        toggle = { LinearMenuToggle } >
        <DropdownToggle color = "secondary"
        caret>
        Linear Algebra { " " } </DropdownToggle>{" "} 
        <DropdownMenu>
        <DropdownItem header > Methods </DropdownItem>{" "} 
        ...
        <DropdownItem href = "/LUDecompose" > { " " }
        LU Decomposition { " " } 
        </DropdownItem>{" "} 
        <DropdownItem href = "/GaussSeidel" > { " " }
        Gauss - Seidel { " " } 
        </DropdownItem>{" "} 
        <DropdownItem href = "/Jacobi" > { " " }
        Jacobi Iteration { " " } </DropdownItem>{" "} 
        </DropdownMenu>{" "} 
        </Dropdown>{" "} 
        </Nav>{" "} 
        </Navbar>{" "} 
